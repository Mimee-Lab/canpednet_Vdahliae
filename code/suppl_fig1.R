...
tidy.tree <- tree %>%
as_tibble() %>%
mutate(sample_origin = if_else(label %in% pop.data$pop,"canPEDnet","Bautista2020")) %>%
as.treedata()
...
pf <- ggtree(tidy.tree) + 
  theme_tree2() +
  geom_aline(aes(color = sample_origin), size = 3)+
  #geom_tiplab() + 
  geom_cladelabel(node=378, label="4B", 
                  color="black", offset=0.01,offset.text = 0.02,hjust = 0.2, align = T) + #\nAB=2\nMB=1\nON=8\nQC=18\nPEI=48\nNB=41
  geom_cladelabel(node=364, label="2A", 
                  color="black", offset=0.01,offset.text = 0.02,hjust = 0.2) +
